...
class Parser:
    ...
    def get_products(self):
        ...
        while True:
            ...
            products = soup.find_all(self.PRODUCT_TAG["tag"], class_=self.PRODUCT_TAG["class_name"])
            
            if not products:
                print("Товары не найдены на странице.")
                break
            for product in products:
                name_elem = product.find(self.PRODUCT_NAME_TAG['tag'], class_=self.PRODUCT_NAME_TAG['class_name'])
                price_elem = product.find(self.PRODUCT_PRICE_TAG['tag'], class_=self.PRODUCT_PRICE_TAG['class_name'])
